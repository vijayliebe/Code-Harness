...
| **Google Code Wiki** | Hosted living docs + grounded chat | Mermaid export, gloss, template `wiki generate` + `path:symbol` WikiPages, `--dirty` / watch hook, opt-in RRF `wiki_weight`, opt-in packer `knowledge/**` prefix | Chat-over-wiki via CCR; LLM polish |

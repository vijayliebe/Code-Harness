...
Fusion PR #3 (interactive session) adds `chat` / `session` / `repl` aliases, JSONL under `.code-harness/sessions/`, heuristic `/compact` + `/cost`, `--profile sage`, and a `path:symbol` system line. Expand-on-explain is session/sage-scoped. Claude Code steal #1 (tool-result clearing) is opt-in on that path (`--clear-tool-results`, default off).

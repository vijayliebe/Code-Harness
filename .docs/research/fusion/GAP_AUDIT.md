...
| **3** | **Interactive session + `/compact` + `/cost`** — JSONL session, budget never drops latest pack, heuristic compact, print packed/full tokens + loop attempts. Tiny: `--profile sage` flag pack + `path:symbol` system line + `expand_on=explain`. *(session + slash cmds + sage + cite line + opt-in tool-result clearing + opt-in default-fail verify gate + opt-in event-session / prefix-stable packing + retrieve-as-pre-step seam + session-event FTS shipped; LLM compact / eval session fixture still open)* | M | Strands #3, Forge #13, Claurst #2, Loop #18, Headroom remainder #1, Claude Code #24, DeepSeek #25 | None (CCR/loop exist) |
...
- **Steal:** Strands never-drop-latest-pack; Forge/Claurst conversation compact + knobs; Loop `path:symbol` instruction; Headroom `expand_on`; Claude Code tool-result clearing (keep ids, stub aged retrieve dumps) + default-fail independent verify gate.
- **Sources:** #3, #13, #2, #18, #1, #24
- **Shipped:** `python main.py chat|session|repl|interactive` is a stateful REPL. JSONL under `.code-harness/sessions/`. Heuristic `/compact` keeps user lines + latest pack ids and never drops the latest pack. Opt-in tool-result clearing (`--clear-tool-results` / `session.clear_tool_results`, default **off**) replaces aged retrieve/`tool_result` payloads with re-fetch placeholders (`chunk_id` / path / tool+args); `/clear-tool-results` is the explicit slash; `/compact` runs it as a micro-step only when the flag is on. Opt-in **default-fail verify gate** (`--verify` / `session.verify` / `CODEHARNESS_SESSION_VERIFY=1`, default **off**) starts completion criteria all-false; only the independent verifier (or `--force-done` / `/done --force`) can accept done. Opt-in **event-sourced session** (`--event-session` / `CODEHARNESS_EVENT_SESSION=1` / `session.event_session`, default **off**) appends typed events (`user` / `assistant` / `tool_use` / `tool_result` / `system` / `compact` / `clear` / `verify` / `meta`); `derive_messages()` is the model-facing history; resume reloads the log; `/compact` and clear are events (raw dumps stay). Implies **prefix-stable packing** (`context.prefix_stable` / `--prefix-stable`): frozen system + sorted tool schemas + path-stable knowledge prefix; volatile hits last. **Retrieve-as-pre-step** (`harness/prestep.py`, default **on**) runs hybrid retrieve+pack as `before_model` on `query` / `chat` / session; `POST /v1/retrieve` shares `run_retrieve_pack`. Disable with `--no-retrieve-prestep` / `CODEHARNESS_RETRIEVE_PRESTEP=0` / `prestep.retrieve: false`, or skip when the caller already supplies packs. **Session-event FTS** (`harness/session_fts.py`) indexes the typed log into a co-located SQLite FTS5 sidecar; `session search "…"` / `/search` / MCP `search_session` return ranked hits (event id + snippet + timestamp). Requires a typed event log (`--event-session`); legacy `event: turn` JSONL is refused. `/cost` prints packed/full/completion tokens + loop attempts + **tool-result tokens freed** when clearing fires (+ $ if `llm.*_usd_per_1m` set). `--profile sage` / `CODEHARNESS_PROFILE` / `/profile sage` is a flag pack (`ccr_lite`, `max_loops=1`, more graph expand, higher pack budget) and does not retune RRF. System prompt cites `` `path:symbol` ``. Session auto-expands omitted chunks on explain/why **or** omitted >50% with no tool loop. `/expand`, `/memory brief`, `/wiki`, `/verify`, `/done` are cheap extras. One-shot `query` is unchanged unless `--profile` is passed. Default turn JSONL is unchanged when the event-session flag is off.
- **Change:** Problem — `interactive` was a stateless one-shot loop; long retrieve dumps bloated history; agents self-graded “done”; mixed JSONL was the only transcript copy; retrieve was inline in query/chat/serve. Outcome — remaining: LLM compact (rejected for v1), a 10-turn eval fixture file, caveman prompt. Session-event FTS **shipped** (DeepSeek steal wave 5/5).
...
### G17. Event-sourced session + prefix-stable packing (P0) — done (remainder: `llm-retry`)

- **Steal:** DeepSeek append-only `Session` + `deriveMessages()`; compaction as surface replacement; byte-identical prefix replay for KV cache. Headroom CacheAligner counted once.
- **Sources:** #25, #1 (CacheAligner — **count once**), not a second Strands budget (#3)
- **Shipped:** `harness/events.py` + `harness/prefix.py`. Opt-in `--event-session` / `CODEHARNESS_EVENT_SESSION=1` / `session.event_session` (default **off**). Typed events `user|assistant|tool_use|tool_result|system|compact|clear|verify|meta`. `derive_messages()` projects the surface; `Session.resume` reloads the log. `/compact` and clear append events (shadow / placeholder); raw dumps stay. Prefix freeze: system sections + sorted tool schemas + path-stable knowledge; `prefix_bytes` identical across two packs with the same config. Legacy turn JSONL + query-ranked knowledge prefix unchanged when flags are off. `migrate_legacy_events()` reads old files. Retrieve-as-pre-step (`harness/prestep.py`) **shipped, default on** — `query` / `chat` / session share `RetrievePreStep`; MCP `/v1/retrieve` uses `run_retrieve_pack` (HTTP payload unchanged). Session-event FTS (`harness/session_fts.py`) **shipped** — SQLite FTS5 sidecar next to the event JSONL; incremental index on append; rebuild on migrate / stale sidecar; `session search` / `/search` / `search_session`. DeepSeek steal wave **5/5 complete**.
- **Change:** Problem — mixed JSONL turns were the only copy; `/compact` mutated the working list; prefix cache was folklore; retrieve was duplicated on query/chat/serve; finding a prior turn meant dumping the transcript. Outcome — remaining: `llm-retry` (not this PR).
- **Metric move:** Tokens/latency on repeated interactive turns **down** when a provider prefix cache hits; resume/fork correctness (unit). Recall@k unchanged.
- **Risk / complexity / local-first:** Medium (migration). Default path unchanged. Do not vendor Cordis.
- **PR size / deps:** M. Unlocks later FTS. Shipped.

...
| **2** | **Typed memory + OKF import/export** — `decision`/`error`/`preference`/`fact`, supersede, `memory brief` ≤800 tokens in the semi-stable prefix. *(store + CLI + brief + OKF + heuristic `memory extract` shipped; packer inject and auto-extract default-off; optional `--llm` refine no-ops without a key)* | M | Memanto #5, OKF #16 | PR 1’s `okf.py` *or* land parser in this PR and have wiki call it |
...
- **Sources:** #5, #16
- **Shipped:** `python main.py memory add|list|brief|export|import|extract` writes OKF `Decision`/`Error`/`Preference`/`Fact` files under `knowledge/memory/` (optional `.code-harness/memory/` overlay). Supersede tombstones the old file; `memory brief` packs active entries ≤800 tokens (`len//4`). Query/CCR inject is `--include-memory-brief` (default off). Heuristic `memory extract` / `/memory extract` reads session JSONL (or a one-shot query trace) and proposes the four kinds with `path:symbol` links; title/link match supersedes. `memory.auto_extract` default **false** (opt-in after `/compact` or session exit). `--llm` refine no-ops without a key. Shared `harness/okf.py` with wiki. See [notes/memory-okf.md](notes/memory-okf.md).
- **Change:** Problem — decisions live in chat or fight code chunks in Chroma. Outcome — remaining: eval fixtures “why Chroma / why pack_mode full”, BM25-over-memory index, prefix-load of all `knowledge/**/*.md`. Full-vault OKF export is next, not this remainder.
...
- **Risk / complexity / local-first:** Medium. Junk-drawer risk — require `type`+`title`. Auto-extract stays opt-in. No Memanto/Mem0 dependency. Heuristic path is offline.

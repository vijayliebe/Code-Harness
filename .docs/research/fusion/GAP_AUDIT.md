...
|---|-----|------|------|------------|
| **1** | **Wiki generate MVP** — template + KG + Mermaid, write `knowledge/wiki/` as OKF `WikiPage`, cite `path:symbol`. No LLM polish. *(generate + schema + `--dirty` / watch hook + opt-in RRF `wiki_weight` + full-vault `knowledge export|import` shipped; chat-over-wiki + LLM polish still open)* | M | Code Wiki #23, OKF #16, Obsidian vault shape #6 | Mermaid (done). Shared `okf.py` with PR 2 |
...
- **Change:** Problem — decisions live in chat or fight code chunks in Chroma. Outcome — remaining: eval fixtures “why Chroma / why pack_mode full”, packer wiring of `load_knowledge_docs`, BM25-over-memory.

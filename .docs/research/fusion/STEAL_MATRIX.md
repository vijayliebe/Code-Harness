...
| 24 | Claude Code harness | partial | P1 | fusion-note |
| 25 | DeepSeek Harness | partial | P0 | fusion-note |
...
- **Status:** `partial` — `harness/memory.py` + CLI `memory add|list|brief|search|export|import|extract` shipped: four types, supersede/tombstone, `memory brief` ≤800 tokens, OKF markdown under `knowledge/memory/`. Heuristic `memory extract` (session JSONL / last session, `--dry-run`, optional `--llm` no-op without client+key). `memory.auto_extract` default **false**; when true, runs after `/compact` or session exit. Query-path inject is default-off (`--include-memory-brief`). Opt-in BM25-over-memory RRF channel via `retrieval.memory_weight` (default **0.0**) or `--include-memory-search` / `CODEHARNESS_MEMORY_SEARCH=1`; `memory search <q>` debugs the same index. Active files only; packer dedupes vs brief / knowledge-prefix. **Delta:** no eval “why did we…?” fixtures yet. Do not vendor Memanto. OKF is the interchange (Google SPEC; Memanto is implementer).
- **Fusion priority:** P0
- **Evidence:** deep-dive
...
- **Status:** `partial` — tool-result clearing **shipped** (opt-in, default **off**). Default-fail verify gate **shipped** (opt-in, default **off**: `--verify` / `CODEHARNESS_SESSION_VERIFY=1` / `session.verify`; `/verify`; `/done --force` / `--force-done`; optional `eval --verify`). Event-sourced session is the DeepSeek sibling (**#25**), not this card. Do not change retrieval defaults.
- **Fusion priority:** P1 (clearing + verify-gate done). Event-session stays later.
- **Evidence:** fusion-note

## 25. DeepSeek Harness (official `dsh`)
...
- **Map to module:** `harness/events.py` (`SessionEvent`, `derive_messages`, `EventLog`, `migrate_legacy_events`); `harness/prefix.py` (frozen section order + tool schemas + path-stable knowledge); `harness/session.py` (`--event-session`); `harness/session_fts.py` (SQLite FTS5 sidecar); `harness/context_builder.py` (`prefix_bytes` / `prefix_stable`); `harness/prestep.py` (`AgentHook` / `RetrievePreStep`).
- **Status:** `partial` — event log + derive + prefix freeze **shipped opt-in** (`--event-session` / `CODEHARNESS_EVENT_SESSION=1` / `session.event_session`, default **off**; implies `context.prefix_stable`). Retrieve-as-pre-step **shipped, default on** (`harness/prestep.py`; `--no-retrieve-prestep` / `CODEHARNESS_RETRIEVE_PRESTEP=0` / `prestep.retrieve: false` skips). Session-event FTS **shipped** (`harness/session_fts.py`; `session search` / `/search` / MCP `search_session`; sidecar `.fts.sqlite` next to the event JSONL). Steal wave **5/5 complete**. Legacy turn JSONL is unchanged. **Delta / later:** `llm-retry`. **Reject:** Cordis megasystem, `@deepseek-ai/*`.
- **Fusion priority:** P0 for event+prefix (shipped). P1 retrieve-pre-step **shipped**. P1 session-event FTS **shipped**.
- **Evidence:** fusion-note
